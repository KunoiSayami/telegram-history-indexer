...
class MediaDownloader:
	...
	async def download(self, file_id: str, file_ref: str) -> None:
		try:
			if await self.conn.query1("SELECT `file_id` FROM `media_store` WHERE `file_id` = %s", file_id) is not None:
				return
			try:
			...
			async with aiofiles.open('downloads/image.jpg', 'rb') as fin:
				#print(file_id)
				await self.conn.execute("INSERT INTO `media_store` (`file_id`, `body`) VALUE (%s, %s)", (file_id, await fin.read()))
		except:
...
class MsgTrackerThreadClass:
	...
	async def _filter_msg(self, msg: Message) -> None:
		if msg.new_chat_members:
			await self.conn.execute("INSERT INTO `group_history` (`chat_id`, `user_id`, `message_id`, `timestamp`) VALUES (%s, %s, %s, %s)",
	...
	def push(self, msg: Message, no_user: bool = False) -> None:
		...
		if no_user: return
		users = [x.raw for x in list(set(UserProfile(x) for x in [msg.from_user, msg.chat, msg.forward_from, msg.forward_from_chat, msg.via_bot]))]
		users.remove(None)
		for x in users:
			self.push_user(x)
...
class check_dup:
	...
	async def check(self) -> None:
		...
		total_count = (await self.conn.query1("SELECT COUNT(*) as `count` FROM `index` WHERE `_id` < %s", last_id))['count']
