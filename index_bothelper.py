...
class sql_cache_thread(threading.Thread):
	...
	def run(self):
		logger.info('sql_cache_thread start successful')
...
class bot_search_helper(object):
	...
	def generate_sql_options(self, args: list, timestamp: str or "datetime.datetime" = '', _type: str = ''):
	...
	def handle_query_media(self, _client: Client, msg: Message):
		...
		args = msg.command[2:]
		_type = msg.command[1] if len(msg.command) > 1 else ''
	...
	def update_max_count(self, _id: int, max_count: int):
		...
		self.conn.execute("UPDATE `query_result_cache` SET `max_count` = %s WHERE `_id` = %s", (max_count, _id))
	...
	@staticmethod
	def get_msg_hash(args: list, _type: str):
		if _type is None: _type = ' '
		return hashlib.sha256((repr(args) + _type).encode()).hexdigest()
